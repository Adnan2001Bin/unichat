              ...
              <>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSearch)} className="space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-medium" style={{ color: THEME_TEXT_DARK }}>Search Users</FormLabel>
                          <div className="flex space-x-2">
                            ...
                            <Button
                              type="submit"
                              disabled={loading}
                              className="font-semibold py-2 px-4 rounded-lg transition-all duration-200 bg-blue-500 hover:bg-blue-600 text-white" // Tailwind colors for bg and hover
                            >
                ...
                <div className="mt-6 space-y-4">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <p className="text-lg font-semibold" style={{ color: THEME_TEXT_DARK }}>{user.userName}</p>
                            {user.university && <p className="text-sm" style={{ color: THEME_TEXT_LIGHT }}>{user.university}</p>}
                            {user.headline && <p className="text-sm italic" style={{ color: THEME_TEXT_LIGHT }}>{user.headline}</p>}
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          {isFriend ? (
                            <span className="flex items-center font-semibold" style={{ color: THEME_ACCENT_GREEN }}>
                              <UserCheck className="w-5 h-5 mr-1" /> Friend
                            </span>
                          ) : isPendingSent ? (
                            <span className="flex items-center font-semibold" style={{ color: THEME_SECONDARY_BLUE }}>
                              <UserPlus className="w-5 h-5 mr-1" /> Request Sent
                            </span>
                          ) : isPendingReceived ? (
                            <span className="flex items-center font-semibold" style={{ color: THEME_PRIMARY_DARK_BLUE }}>
                              <UserX className="w-5 h-5 mr-1" /> Request Received
                            </span>
                          ) : (
                            <Button
                              onClick={() => handleSendRequest(user._id)}
                              className="font-semibold py-2 px-4 rounded-lg transition-all duration-200 bg-yellow-500 hover:bg-yellow-600 text-white" // Tailwind colors for bg and hover
                            >
                              Send Friend Request
                            </Button>
                          )}
                  ...
                  {searchResults.length === 0 && !loading && form.formState.isSubmitted && (
                    <p className="text-center text-lg py-8 animate-fade-in" style={{ color: THEME_TEXT_LIGHT }}>
                      No users found
                    </p>
                  )}
                </div>
