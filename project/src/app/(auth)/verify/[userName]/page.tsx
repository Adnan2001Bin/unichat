...
export default function VerifyCodePage() {
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Error resending code.';
  ...
  const onSubmit = async (data: VerifyCodeInput) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Error verifying code. Please try again.';
      toast.error('Error', {
        description: errorMessage,
        className: 'bg-red-600 text-white border-red-700 backdrop-blur-md bg-opacity-80',
        duration: 4000,
        ...
        <Card className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
          <CardHeader className="text-center space-y-3">
            ...
            <CardDescription className="text-gray-600 text-base">
              ...
              <br />
              <span className="font-semibold text-blue-600">{username ? `for ${username}` : ''}</span>
            </CardDescription>
          </CardHeader>
