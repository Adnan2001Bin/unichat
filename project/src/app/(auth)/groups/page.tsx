...
import { Search, ImageIcon, Users, PlusCircle, CheckCircle2, XCircle } from 'lucide-react';
...
const GroupsPage: React.FC = () => {
  ...
  const fetchGroups = async (query: string = '', page: number = 1) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/groups?query=${encodeURIComponent(query)}&page=${page}&limit=10`);
  ...
  const handleManageRequest = async (groupId: string, userId: string, action: 'approve' | 'reject') => {
    try {
  ...
  const handleJoinGroup = async (groupId: string, privacy: 'public' | 'private') => {
    try {
      const response = await fetch('/api/groups/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ groupId, action: privacy === 'public' ? 'join' : 'request' }),
      });
      const result = await response.json();
      if (result.success) {
        toast.success(result.message, {
  ...
  const createdGroups = groups.filter((group) => group.creator.userName === session?.user?.userName);
  ...
  return (
    ...
    >
      ...
      <Card
        ...
        style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)' }} // Still slightly transparent white for card bg
      >
        <CardContent className="p-0">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6">
            <h1 className="text-4xl font-extrabold text-center sm:text-left" style={{ color: THEME_TEXT_DARK }}>
              <span className="bg-clip-text text-transparent" style={{ backgroundImage: `linear-gradient(to right, ${THEME_PRIMARY_DARK_BLUE}, ${THEME_ACCENT_GREEN})` }}>
                Student Connect
              </span>{' '}
              Groups
            </h1>
            ...
            >
              <PlusCircle className="inline-block mr-2" size={24} /> Create New Group
            </Button>
          </div>
          ...
          <form onSubmit={handleSearch} className="mb-10 flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400 w-6 h-6" style={{ color: THEME_TEXT_LIGHT }} />
              <Input
                value={searchQuery}
          ...
          {pendingRequests.length > 0 && (
            <>
              ...
              <h2 className="text-3xl font-bold mb-8 text-center" style={{ color: THEME_TEXT_DARK }}>
                Requests Awaiting Your Review
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                {pendingRequests.map((request) => (
                  <Card key={request.groupId} className="bg-white shadow-xl rounded-2xl border border-gray-100 transform hover:scale-103 transition-transform duration-300">
                    <CardContent className="p-6">
                      <h3 className="text-2xl font-bold mb-4" style={{ color: THEME_TEXT_DARK }}>
                        {request.groupName}
                      </h3>
                      {request.pendingUsers.length === 0 ? (
                        <p className="text-gray-500 italic text-center py-4">No pending requests</p>
                      ) : (
                        <ul className="space-y-4">
                          {request.pendingUsers.map((user) => (
                            <li key={user.userId} className="flex flex-col sm:flex-row justify-between items-center bg-gray-50 p-4 rounded-xl shadow-inner border border-gray-100">
                              <p className="text-lg font-medium" style={{ color: THEME_TEXT_LIGHT }}>
                                {user.userName}
                              </p>
                              <div className="flex gap-3 mt-3 sm:mt-0">
                                <Button
                                  onClick={() => handleManageRequest(request.groupId, user.userId, 'approve')}
                                ...
                                <Button
                                  onClick={() => handleManageRequest(request.groupId, user.userId, 'reject')}
          ...
          {joinedGroups.length === 0 ? (
            <p className="text-center py-12 text-xl bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 mb-12 animate-fade-in" style={{ color: THEME_TEXT_LIGHT }}>
                  ...
                  <Link href={`/groups/${group._id}`} className="block">
                    <div className="h-48 w-full bg-gray-100 relative overflow-hidden">
                      {group.coverImage ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-gray-600" style={{ backgroundColor: THEME_PRIMARY_DARK_BLUE }}>
                          <ImageIcon className="h-20 w-20 text-white opacity-70" />
                          <div className="absolute inset-0 bg-black opacity-10"></div>
                        </div>
                      )}
                      <div className="absolute bottom-3 right-3 bg-white text-xs font-semibold px-3 py-1 rounded-full shadow-md flex items-center" style={{ color: THEME_TEXT_DARK }}>
                        <Users className="inline-block mr-1" size={14} /> {group.memberCount}
                      </div>
                    </div>
                  </Link>
                  <CardContent className="p-6">
                    <Link href={`/groups/${group._id}`} className="block">
                      <h2 className="text-2xl font-bold mb-2 truncate" style={{ color: THEME_TEXT_DARK }}>
                        {group.name}
                      </h2>
                      <p className="text-md line-clamp-2 mb-4" style={{ color: THEME_TEXT_LIGHT }}>
                        {group.description}
                      </p>
                    </Link>
                    <div className="flex justify-between items-center">
                      <span className="font-bold px-4 py-1 rounded-full text-white text-sm" style={{ backgroundColor: THEME_ACCENT_GREEN }}>
                        Joined
                      </span>
                    </div>
                  </CardContent>
              ...
              {createdGroups.map((group) => (
                ...
                >
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="font-bold px-4 py-1 rounded-full text-white text-sm" style={{ backgroundColor: THEME_PRIMARY_DARK_BLUE }}>
                        Creator
                      </span>
                    </div>
                  </CardContent>
          ...
          {groups.length === 0 ? (
            <p className="text-center py-12 text-xl bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 mb-12 animate-fade-in" style={{ color: THEME_TEXT_LIGHT }}>
              No groups match your search. Unleash your creativity and create a new one!
            </p>
          ) : (
                      ...
                      {!group.isMember ? (
                        ...
                        >
                          {group.privacy === 'public' ? (
                            <>
                              <PlusCircle size={18} className="mr-2" /> Join Group
                            </>
                          ) : (
                            <>
                              <PlusCircle size={18} className="mr-2" /> Request to Join
                            </>
                          )}
                        </Button>
                      ) : (
